...
const Add = () => {
    ...
    const updatedTableData = tableData.map((item) => {
      if (item.key === record.key) {
        if (type === "start") {
          return {
            ...item,
            sale_price_start_at: date ? date.format("YYYY-MM-DD") : null,
          };
        }
        if (type === "end") {
          return {
            ...item,
            sale_price_end_at: date ? date.format("YYYY-MM-DD") : null,
          };
        }
      }
      return item;
  ...
  const generateVariants = () => {
    ...
    const attributesWithValues = forms.filter(
      (form) => form.name && form.values.length > 0
    );
          ...
          rules={[
            {
              validator: (_, value) =>
                record.thumbnail
                  ? Promise.resolve()
                  : Promise.reject("Vui lòng tải lên ảnh"),
            },
          ]}
        >
          <Upload
    ...
    },
    {
      ...
      render: (text, record) => (
        <DatePicker
          format="DD-MM-YYYY"
          value={
            record.sale_price_start_at
              ? dayjs(record.sale_price_start_at)
              : null
          }
    ...
    },
    {
      ...
      render: (text, record) => (
        <DatePicker
          format="DD-MM-YYYY"
          value={
            record.sale_price_end_at ? dayjs(record.sale_price_end_at) : null
          }
        ...
        <Row gutter={24}>
          ...
          {productType === "single" && (
            <>
              ...
              <Col span={8} className="col-item">
                <Form.Item label="Giá khuyến mại (VNĐ)" name="sale_price">
                  <InputNumber className="input-item" />
                </Form.Item>
        ...
        {productType === "variant" && (
          <>
                  ...
                  onChange={(values) => {
                    if (values.includes("create_value")) {
                      ...
                      const filteredValues = values.filter(
                        (value) => value !== "create_value"
                      );
                      forms[index].values = filteredValues.map((value) => ({
                        id: Number(value),
                        value:
                          filteredValues.find((val) => val.id === Number(value))
                            ?.value || "",
                      }));
                    } else {
            ...
            <Button
              ...
              className="btn-item"
              onClick={() =>
                setForms([...forms, { id: Date.now(), name: "", values: [] }])
              }
            >
              Thêm thuộc tính
            </Button>
              ...
              rules={[
                {
                  validator: (_, value) =>
                    logo
                      ? Promise.resolve()
                      : Promise.reject("Vui lòng tải lên ảnh thương hiệu"),
                },
              ]}
            >
              <Upload

...
const Edit = () => {
    ...
    addAttributeMutation.mutate(newAttr, {
      onSuccess: () => {
        notification.success({
          message: "Thuộc tính đã được thêm vào phần biến thể",
        });
      },
    });
  };
  ...
  const handleAddValue = (formId) => {
    const updatedForms = forms.map((form) => {
      if (form.id === formId) {
        if (!form.name) {
          notification.error({
            message: "Vui lòng chọn thuộc tính trước khi thêm giá trị",
          });
          return form;
        }
  ...
  const handleDeleteTag = (formId, valueToDelete) => {
    const updatedForms = forms.map((form) => {
      if (form.id === formId) {
        return {
          ...form,
          values: form.values.filter((value) => value !== valueToDelete),
        };
      }
      return form;
    });
  ...
  const handleAddVariantForm = () => {
    setForms([
      ...forms,
      { id: Date.now(), name: "", values: [], inputValue: "" },
    ]);
  };
  ...
  const generateVariants = (forms) => {
    if (
      forms.length === 0 ||
      forms.some((form) => !form.name || form.values.length === 0)
    ) {
      setVariants([]);
      return;
    }
            ...
            <div key={form.id}>
              <div className="attribute">
                <Select
                  ...
                  onChange={(value) => {
                    if (
                      forms.some((f) => f.name === value && f.id !== form.id)
                    ) {
                      notification.error({
